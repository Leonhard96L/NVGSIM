...
from function_lib import ft2m, map360, map_control, pitch_brun2N, roll_brun2N, roll_brun2angle, pitch_brun2angle, \
...
def log_flyout_input_output(T):
    ...
    while i < len(T) - 1:
        ...
        length, status, node_pitch, force_pitch, node_roll, force_roll = struct.unpack('<HBHfHf',
                                                                                       force_response_pitch_roll)
    ...
    for count, param in enumerate(params):
        ...
        if 'FTD1' in data.keys():
            ...
            if 'Yaw Angle Unwrapped' in para_file_dict[plot_title]:
                ...
                y = np.rad2deg(y)
                y_Rec = np.rad2deg(y_Rec)
            elif 'Control Position Pitch' in para_file_dict[
                plot_title]:  # Pitch position Signal ist bei der Referenz invertiert
                y = [map_control(-i) for i in y]
                y_Rec = [map_control(-i) for i in y_Rec]
