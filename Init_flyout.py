...
def logandsave_flyout_init_cond(QTG_path, filename):
    ...
    init_cond_FTD = os.path.join(QTG_path, filename_date)
    
    with open(init_cond_FTD, 'w') as json_file:
        json.dump(init_cond_log_dict, json_file, indent=4)
...
def set_init_cond_flyout(init_cond_dict):
    ...
    if init_cond_dict['AFCS State'] == 'DSAS':
        ...
        flightmodel_module_simple_scas_input_scas_pitch_on.write(False)
        flightmodel_module_simple_scas_input_scas_roll_on.write(False)
        flightmodel_module_simple_scas_input_scas_yaw_on.write(False)

    
    
    
def get_QTG_init_cond_ref(QTG_path):
    init_cond_ref_path = os.path.join(QTG_path, 'Reference_init_cond.json')
    with open(init_cond_ref_path, 'r') as json_file:
        init_cond_ref_dict = json.load(json_file)
    return init_cond_ref_dict
...
if __name__ == "__main__":
    ...
    logandsave_flyout_init_cond(QTG_path,'FTD1_log_init_cond')
...
-Eine App machen oder Webapp -> muss fuer MQTG beschrieben werden
    ...
    9. SChreibe die Beschreibung aller Tests ab 
...
Noch unsolved:
    ...
    Zeichne die Eingabe krefte aller Controls fuer alle Tests auf.
    -Multisim die EC135 CLS und beep trim als aufgabe geben
    -Funktion nach jedem test aufrufen, set_standard_values()
    -set_clock to 0
